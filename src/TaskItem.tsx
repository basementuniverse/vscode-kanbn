...
const TaskItem = ({
  ...
  // Nothing in the card body depends on the drag state, so a read-only board can render exactly the
  // same card without a Draggable around it
  const content = (
    <React.Fragment>
  ...
  const className = (isDragging: boolean) => [
    'kanbn-task',
  ...
  // A view's columns are filters rather than index columns, so there's nowhere for a drag to land -
  // dragging a card between them would have no meaning to move the task to
  if (!draggable) {
    return (
      <div className={className(false)} style={{ userSelect: "none" }}>
        {content}
      </div>
    );
  }
