...
* Kanbn no longer fails to activate when the workspace folder isn't one it can work in. It reads and writes board files through the filesystem, so a virtual workspace — github.dev, vscode.dev, or a repository opened with the GitHub Repositories extension — has no folder for it to work in, and a folder can also be deleted or unmounted while the window stays open. Either used to throw out of activation, which took the whole extension down with it: no status bar item, and no explanation anywhere. The commands now say what's wrong, and the extension declares that it needs a filesystem so VS Code disables it in a virtual workspace rather than letting it fail. Remote-SSH, WSL, dev containers and Codespaces are unaffected — the extension host runs alongside the files there, which is an ordinary filesystem as far as kanbn is concerned

# 1.2.1

* Fixed the extension reporting "Not initialised in this folder" on a board that is perfectly well initialised, with no Kanbn status bar item to be seen. A target board is resolved from the `KANBN_BOARD` environment variable and then the `defaultBoard` option before anything is opened, and 1.2.0 scoped every operation to whatever came back without checking that the board existed — so a variable left over in a shell, or a `defaultBoard` naming a board that was never created or has since been deleted, made the whole extension look broken. A target board that doesn't exist now falls back to the main board and says which board it was looking for. This was easier to hit on macOS and Linux than on Windows, because VS Code resolves the login shell's environment there and the extension host inherits variables set in `.zshrc` or `.bashrc`
* The Kanbn status bar item no longer disappears when a board fails to load. It refreshed by loading the target board's status, and any error — a missing board, an index that doesn't parse — left the item hidden rather than merely uninformative, which reads as the extension not being installed. It now falls back to showing the Kanbn icon with the error in its tooltip, and still opens the board when clicked
* A board file that isn't there is now reported as such, rather than as the workspace not being initialised. Kanbn reports both the same way, which sent people off to initialise a board they already had
